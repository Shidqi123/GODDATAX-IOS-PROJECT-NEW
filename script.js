...
function installDNS() {
  console.log('📡 Initiating DNS Profile installation...');
  
  // Set flag that DNS installation started
  localStorage.setItem('dnsStepStarted', 'true');
  
  // Update UI State
  const step1 = document.getElementById('step1');
  if (step1) step1.classList.add('active-install');

  showNotification('Mengunduh Profil DNS. Silakan klik "Izinkan".');
  
  // Link ke file mobileconfig
  setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    showNotification('Silakan ke Pengaturan > Profil Diunduh untuk memasang.');
  }, 2500);
}

function installIPA() {
  console.log('📲 Initiating IPA installation...');
  
  const manifestUrl = window.location.origin + window.location.pathname.replace('index.html', '') + 'manifest.plist';
  const itmsUrl = `itms-services://?action=download-manifest&url=${manifestUrl}`;
  
  localStorage.setItem('saiSetupCompleted', 'true');
  
  // 1. Picu sistem download iPhone
  window.location.href = itmsUrl;
  
  // 2. Beri waktu sistem untuk memproses popup download
  setTimeout(() => {
    // Mencoba menutup tab (hanya berhasil di beberapa kondisi PWA)
    window.close();
    
    // 3. Fallback: Mengalihkan ke halaman kosong jika tutup gagal
    // Ini memberikan kesan website sudah "berhenti" bekerja
    setTimeout(() => {
